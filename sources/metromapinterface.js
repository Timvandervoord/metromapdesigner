...
export async function loadInterface() {
  try {
    ...
    const metrolineColors = metromapdesignapplication.getAllMetrolineColors();
    if (metrolineColors && metrolineColors.length > 0) {
      useMetrolineTool(helpers.convertToRgb(metrolineColors[0]));
    } else {
      console.warn('No metroline colors available, using default color');
      useMetrolineTool(helpers.convertToRgb('#1E90FF')); // Default blue color
    }
  } catch (e) {
...
// Throttled version for event listeners (~60fps)
const throttledDragInterfaceItem = helpers.throttle(dragInterfaceItem, 16);
...
function initInterface() {
  ...
  // Document-wide event listeners for dragging (throttled for performance)
  document.addEventListener("mousemove", throttledDragInterfaceItem);
  document.addEventListener("touchmove", throttledDragInterfaceItem);
